function extract_multilayer_power_changes_par(sFiles, protocol_name, params)
...
% Translate to short filenames 
sFiles_short = file_short(sFiles);
...
% Identify jobs which need the same filtering to reduce computation demands
pair = zeros(n_jobs, 2);

for i = 1:n_jobs
    tok = regexp(jobs{i}, '/\s*([\d]+),\s*([\d]+)\s*/', 'tokens', 'once');
    pair(i, :) = str2double(tok);
end

[G, ~] = findgroups(pair(:, 1), pair(:, 2));
...
for g = 1:numel(G)
    ...
    if ~isempty(idx)
        independend_jobs(end+1, :) = unique(pair(idx, :));
        n_sub_jobs(end+1) = size(pair(idx, :), 1);
    end
end

fprintf('Number of independend jobs: %i\n', size(independend_jobs, 1));

sfreq = round(1/(time(2) - time(1)));
%% Check if all files are of the same run
nTrialsTotal = length(sFiles_short);
ses_ids = unique(cellfun(@fileparts, sFiles_short, 'UniformOutput', false));
% group files in sessions
session_files = cell(1, length(ses_ids));
session_files_short = cell(1, length(ses_ids));
for ses = 1:length(ses_ids)
    file_mask = cellfun(@(x) contains(x, ses_ids{ses}), sFiles);
    session_files{ses} = {sFiles{file_mask}};
    session_files_short{ses} = {sFiles_short{file_mask}};
end
...
for ses=1:length(ses_ids)
    ...
    disp('Starting extraction (With parfor. No progress update possible) ...')
    %fprintf('Progress: %3d%%\n', 0);
    parfor iTrial = 1:sesTrials
        %fprintf(1, '\b\b\b\b%3.0f%%', 100*(iTrial/sesTrials));

        trial_template = ses_template;

        % Load data for each trial
        [~, trial_id] = fileparts(session_files_short{ses}{iTrial});
        sRawData = load(session_files{ses}{iTrial});
        ...
        for job = 1:size(independend_jobs, 1)
            ...
            for sub_job=1:n_sub_jobs(job)
                ...
                % Update job idx to store data
                job_idx = job_idx + 1;
            
                %% Initialize current extraction params
                job_specs = split(jobs{job_idx}, '/');
        ...
        trial_template.DataFile = char(sprintf('link|%s|%s', kernel_fname, session_files_short{ses}{iTrial}));
        
        out_fname = sprintf('timefreq_%s_psd_multilayer_power_change_par.mat', trial_id(end-7:end));
        
        save(fullfile(bstrm_out_path, out_fname), '-fromstruct', trial_template);
    end % iTrial
